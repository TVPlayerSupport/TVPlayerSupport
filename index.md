...
- 支持文件的拖放操作。
- 支持播放文件列表。
- 自动保存最新打开或拖入的直播源文件，如需要更新，编辑新文件后，再次打开或拖入播放窗口即可。
- 窗口大小从480x320到全屏可调整。
...
- 电视盒子+显示器，硬件价格在千元左右；还需要数据线、摆放显示器的空间。广播电视直播源Player恐怕比一根数据线的价格还便宜，如今京东的快递费都涨到15块了。可以替代电视盒子和显示器，无需额外空间、电源及数据连线，省钱省事省空间。
...
- 不保证所有源都有效、流畅。因为有些代理源开始很流畅，过一天或几个小时，就变得很卡，然后失效，俗称短命源。
- 个人精力有限，不承诺长期更新。目前，自己也用这个Player看电视，因此，大概可以不定期更新。
- 要想找到好源，需要经常泡坛子，某些好源瞬间即失。
- 播放卡顿的问题：我用VLC专业播放器，进行过一些对比测试。在播放同一源时，流畅的都流畅，卡的都卡，主要还是源的问题。
  为此，还在新浪博客上发了博文，可惜，我的新浪博客被永久封闭。其余国内平台大都如此，有下载链接的博文，基本秒删，不封就算客气的啦。
  不得已，只能在GitHub上做网页，但境外服务器，国内访问速度有时比较慢，只能说比没有强。

## 源地址模版文件

- 为了您的使用方便，特制作了源地址模版文件，需要的可下载，包括央视等电视电影，[查看详情请点击](http://TVPlayerSupport.github.io/Details/)。
- 建议您下载模版文件后，更改文件名，以便区分以后下载的模版文件。
- 新增源地址，先放在新增目录下。如果没有失效，再次更新增加地址时，原地址将移动到相应分类目录下，敬请留意。
