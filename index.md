...
## 版本9.1(待审核)新增播放列表文件格式转换工具

为了便于txt格式和m3u格式的播放列表文件之间进行格式转换，开发了一个格式转换工具，可以将txt格式的播放列表文件转换为m3u格式的播放列表文件，或将m3u格式的播放列表文件转换为txt格式的播放列表文件，具体操作如下：

- 点击菜单“播放控制”，点击“显示/隐藏格式转换工具”，将出现格式转换窗口。
- 选择打开txt格式文件，另存为m3u格式文件。
- 或选择打开m3u格式文件，另存为txt格式文件。

## 版本9.0 m3u读取方式改进介绍

- 一个俄罗斯用户给我发了邮件，提到播放器无法正确读取他的m3u列表文件，并将其文件发送给我。我测试后发现其m3u文件中几处多了一行，包含有#EXTVLCOPT关键字，原版本中没有对包含有#EXTVLCOPT的行进行处理，因此，进行了版本改进。
- 如果您也发生过类似情况，请检查一下m3u文件格式是否与下面模版文件格式一致，如果有差异，可以适当修改，然后再试试是否能正确读取。同时希望您能将无法正确读取的m3u列表文件发送给我，我测试后将对软件进行改进。
